...
def has_any(tag_str: pd.Series, keywords) -> pd.Series:
    ...
    # match whole tokens loosely; simple substring is OK here
...
def build_full(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Extended nutrition (kcal + macros) normalized
    ...
    # Simple text-derived signals from ingredients
    ingr = df.get("ingredients_tags", pd.Series(index=df.index))
    n_ingredients = count_ingredients(ingr)
    has_nuts       = has_any(ingr, ["nut", "almond", "hazelnut", "peanut", "walnut", "cashew", "pistachio"])
    has_chocolate  = has_any(ingr, ["chocolate", "cocoa"])
    has_seed       = has_any(ingr, ["seed", "sesame", "flax", "chia", "poppy", "sunflower"])
    has_fruit      = has_any(ingr, ["fruit", "apple", "berry", "grape", "raisin", "banana", "apricot", "fig"])
    ...
    # Keep identifiers for joins/debug
...
# automated CLI because i am too lazy to do them in the terminal
def main():
    ap = argparse.ArgumentParser(description="Build FULL vectors for downstream ML.")
